...
## 1.尽量减少HTTP请求数【内容】 {#1.尽量减少HTTP请求数【内容】}
...
## 2.使用CDN（Content Delivery Network）【服务器】 {#2.使用CDN（ContentDeliveryNetwork）【服务器】}
...
## 3.添上Expires或者Cache-Control HTTP头【服务器】 {#3.添上Expires或者Cache-ControlHTTP头【服务器】}
...
Gzipping一般能够把响应压缩到70%左右，目前大约90%的通过浏览器的网络传输都支持gzip。如果是Apache服务器，配置gzip的模块取决于版本：Apache 1.3用mod\_gzip而Apache 2.x是mod\_deflate模块。
...
## 7.避免使用CSS表达式【css】 {#7.避免使用CSS表达式【css】}
...
## 8.把JavaScript和CSS放到外面【js、css】 {#8.把JavaScript和CSS放到外面【js、css】}
...
## 10.压缩JavaScript和CSS【js、css】 {#10.压缩JavaScript和CSS【js、css】}
...
## 23.给Cookie减肥【Cookie】 {#23.给Cookie减肥【Cookie】}
...
## 24.把组件放在不含cookie的域下【Cookie】 {#24.把组件放在不含cookie的域下【Cookie】}
...
## 25.尽量减少DOM访问【js】 {#25.尽量减少DOM访问【js】}
...
## 27.选择`<link>`舍弃`@import`【css】 {#27.选择link舍弃@import【css】}

前面提到了一个最佳实践：为了实现逐步渲染，CSS应该放在顶部。

在IE中用`@import`与在底部用`<link>`效果一样，所以最好不要用它。

## 28.避免使用滤镜【css】 {#28.避免使用滤镜【css】}
...
## 30.优化CSS Sprite【图片】 {#30.优化CSSSprite【图片】}
...
## 31.不要用HTML缩放图片【图片】 {#31.不要用HTML缩放图片【图片】}

不要因为在HTML中可以设置宽高而使用本不需要的大图。如果需要  
`<img width="100" height="100" src="mycat.jpg" alt="My Cat" />`  
那么图片本身（mycat.jpg）应该是100x100px的，而不是去缩小500x500px的图片。

## 32.用小的可缓存的favicon.ico（P.S. 收藏夹图标）【图片】 {#32.用小的可缓存的favicon.ico（P.S.收藏夹图标）【图片】}
...
原文：[http://www.ayqy.net/blog/前端优化：雅虎35条](http://www.ayqy.net/blog/前端优化：雅虎35条/?utm_source=tuicool)/
